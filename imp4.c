...
int main() {
    ...
    for (t = 1; t < NT - 1; t++) {
         #pragma omp parallel for num_threads(8) private(i, j, k, laplacianoX, laplacianoY, laplacianoZ, laplacianoT) collapse(3)
        for (i = 2; i < NX - 2; i++) {
            for (j = 2; j < NY - 2; j++) {
                for (k = 2; k < NZ - 2; k++) {
        ...
        #pragma omp parallel for num_threads(8) private(i, j, k) collapse(3)
        for (i = 2; i < NX - 2; i++) {
            for (j = 2; j < NY - 2; j++) {
                for (k = 2; k < NZ - 2; k++) {
