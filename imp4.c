...
#define PRINT_INTERVAL 1 // Intervalo de impressão da propagação da onda
...
float s(float t, float xs, float ys, float zs) {
    ...
    float t0 = 1.0 / f; // Calcula o tempo de atraso t0 da onda sísmica
    float t_delay = t - t0; // Calcula o tempo de atraso da onda sísmica
...
int main() {
    ...
    for (i = 0; i < NX; i++) {
        for (j = 0; j < NY; j++) {
            for (k = 0; k < NZ; k++) {
                s_values[i][j][k] = s(0, i * DELTA_X, j * DELTA_Y, k * DELTA_Z);
                u_past[i][j][k] = s_values[i][j][k];
                u[i][j][k] = s_values[i][j][k];
                u_future[i][j][k] = s_values[i][j][k];
            }
        }
    }
    start = omp_get_wtime();
...
for (t = 1; t < NT - 1; t++) {
    #pragma omp parallel for collapse(3) private(i, j, k, laplacianoX, laplacianoY, laplacianoZ, laplacianoT)
    for (i = 2; i < NX - 2; i++) {
        for (j = 2; j < NY - 2; j++) {
            for (k = 2; k < NZ - 2; k++) {
    ...
    FILE* file = fopen("wave_data.txt", "w"); // Abre o arquivo para escrita

    for (i = 0; i < NX; i++) {
        for (j = 0; j < NY; j++) {
            for (k = 0; k < NZ; k++) {
                fprintf(file, "%f ", u[i][j][k]); // Salva os valores no arquivo
            }
            fprintf(file, "\n");
        }
        fprintf(file, "\n");
