...
int main() {
    ...
    for (t = 1; t < NT - 1; t++) {
         #pragma omp parallel for num_threads(8) private(i, j, k, laplacianoX, laplacianoY, laplacianoZ, laplacianoT) shared(u, u_past, u_future)
        for (i = 2; i < NX - 2; i++) {
            for (j = 2; j < NY - 2; j++) {
                for (k = 2; k < NZ - 2; k++) {
        ...
        #pragma omp parallel for num_threads(8) private(i, j, k) shared(u, u_past, u_future)
        for (i = 2; i < NX - 2; i++) {
            for (j = 2; j < NY - 2; j++) {
                for (k = 2; k < NZ - 2; k++) {
